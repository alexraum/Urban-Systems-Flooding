...
df = pd.read_csv('multivariate_data.csv', index_col = 'PROPERTY_ID', header = 0)
...
# plot average percentage change in the months following Matthew (orange)
x_matt = X[0:POST_MATT]; y_matt = avg_Y[0:POST_MATT]
ax.scatter(x_matt, y_matt, label='post-Matthew', s=MARK_SIZE , color="#fc4e2a")

# plot average percentage change in the months following Florence (blue)
x_flor = X[POST_MATT:POST_FLOR]; y_flor = avg_Y[POST_MATT:POST_FLOR]
ax.scatter(x_flor, y_flor, label='post-Florence', s=MARK_SIZE, color="#3182bd")
