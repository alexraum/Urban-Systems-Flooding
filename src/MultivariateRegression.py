...
df = pd.read_csv('water_proximity.csv', index_col = 'PROPERTY_ID', header=0)
#df = pd.read_csv('joined_data.csv', index_col = 'PROPERTY_ID', header=0)

# remove all missing values
df = df.dropna(axis=0)

# tranform appropriate data using a logarithmic transformation
transVars = ['SQUARE_FOOTAGE', 'DISTANCE_FROM_WATER']
...
predictors = ['MONTHS_POST_FLOOD', 'YEAR_BUILT',
              'SQUARE_FOOTAGE', 'CUMUL_FLOODS', 'DISTANCE_FROM_WATER']
...
for idx in range(NUM_OBS):
    ...
    # create a 'zone' variable for formatting plot
    zone = str(int((idx + 2) / 2))
